...
def format_state_for_llm(state_data, include_debug_info=False, include_npcs=True, use_json_map=False):
    ...
    return format_state(state_data, format_type="detailed", include_debug_info=include_debug_info,
                        include_npcs=include_npcs, use_json_map=use_json_map)
...
def _format_map_info(map_info, player_data=None, include_debug_info=False, include_npcs=True, full_state_data=None,
    ...
    if use_json_map:
        ...
        if not json_generated and 'tiles' in map_info and map_info['tiles']:
            ...
            if (0 <= center_y < len(raw_tiles) and raw_tiles and
                    0 <= center_x < len(raw_tiles[center_y]) and raw_tiles[center_y][center_x]):
            ...
            if npcs:
                for npc in npcs:
                    ...
                    try:
                        ...
                        continue
                    if (abs(nx_int - player_x) <= radius + 1 and
                            abs(ny_int - player_y) <= radius + 1):
                        npc_lookup[(nx_int, ny_int)] = {
            ...
            for y_idx, row in enumerate(raw_tiles):
                for x_idx, tile_data in enumerate(row):
                    ...
                    if (player_tile_symbol in {'S', 'D'} and symbol in {'#', 'W'}
                            and player_coords and abs(game_x - player_x) + abs(game_y - player_y) == 1):
                        tile_walkable = True
                        if tile_type == "blocked":
                            tile_type = "warp_exit"

                    npc_info = npc_lookup.get(coord_key)
                    if npc_info:
                        tile_type = "npc"
                        tile_walkable = False
    ...
    try:
        ...
        if full_state_data and 'location_connections' in full_state_data:
            ...
            if location_name in location_connections:
                ...
                for other_loc, my_coords, their_coords in location_connections[location_name]:
                    lines.append(
                        f"  At ({my_coords[0]}, {my_coords[1]}) → {other_loc} ({their_coords[0]}, {their_coords[1]})")
        ...
        if map_id_connections:
            ...
            if current_map_id and current_map_id in map_id_connections:
                ...
                print(
                    "No connections found for map ID {current_map_id} (available: {list(map_id_connections.keys()) if map_id_connections else 'None'})")
        else:
